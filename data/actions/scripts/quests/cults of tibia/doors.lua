...
local posAcessSoul = Position(32508, 32370, 9)
	...
	if item:getPosition() == posAcessMinotaurs then
		if player:getStorageValue(Storage.CultsOfTibia.Minotaurs.Acesso) >= 0 then
			player:teleportTo(toPosition, true)
			item:transform(item.itemid + 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The door seems to be sealed against unwanted intruders.")
	...
	if item:getPosition() == posBossMinotaurs then
		if player:getStorageValue(Storage.CultsOfTibia.Minotaurs.Mission) >= 0 then
			player:teleportTo(toPosition, true)
			item:transform(item.itemid + 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The door seems to be sealed against unwanted intruders.")
	...
	if item:getPosition() == posAcessInvestigation then
		if player:getStorageValue(Storage.CultsOfTibia.MotA.Mission) >= 0 then
			player:teleportTo(toPosition, true)
			item:transform(item.itemid + 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The door seems to be sealed against unwanted intruders.")
	...
	if item:getPosition() == posAcessSand then
		if player:getStorageValue(Storage.CultsOfTibia.Life.Mission) >= 0 then
			player:teleportTo(toPosition, true)
			item:transform(item.itemid + 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The door seems to be sealed against unwanted intruders.")
	...
	if item:getPosition() == posAcessSoul then
		if player:getStorageValue(Storage.CultsOfTibia.Misguided.Mission) >= 0 then
			player:teleportTo(toPosition, true)
			item:transform(item.itemid + 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The door seems to be sealed against unwanted intruders.")
