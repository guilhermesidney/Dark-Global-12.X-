...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local playerTile = Tile(Position(x, 32661, 6)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.TheSecretLibrary.TheOrderOfTheCobra.CobraTimer) > os.time() then
	...
	for x = 33394, 33396 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.TheSecretLibrary.TheOrderOfTheCobra.CobraTimer, os.time() + 20 * 60 * 3600) -- + 20 * 60 * 3600
