...
local function hasTouchedOneThrone(player)
	for i = 1, #throneStorages do
		if player:getStorageValue(throneStorages[i]) == 0 then
			return true
		end
	end
	return false
...
function onStepIn(creature, item, position, fromPosition)
	...
	if not hasTouchedOneThrone(player) or player:getLevel() < 100 or player:getStorageValue(Storage.TheInquisition.Questline) < 0 then
